maps<-function(object,mode="driving",arrival="21600",auth=NULL,region=NULL){
  ...
  from <- paste(object[,1],collapse="|")
  to <- paste(object[,2],collapse="|")
  ...
  if (length(strsplit(u,split="")[[1]])>1950) {
    stop(gettext("Number of directions too large."))
  }
  ...
  for (i in 1:nrow(object)){
    dist <- append(dist,dat$rows[[i]]$elements[[1]]$distance$value/1000)
    dur <- append(dur,dat$rows[[i]]$elements[[1]]$duration$value/3600)
  }
  close(f)
